...
contributionsCard.innerHTML = `${GAMES_JSON.reduce((acc, game) => acc + game.backers, 0).toLocaleString()}`;
...
const displayStr =
    `A total of of $${amountRaised.toLocaleString()} has been raised for ${GAMES_JSON.length} game${GAMES_JSON.length === 1 ? '' : 's'}.` +
...
firstGameEl.innerText = first.name;
firstGameContainer.appendChild(firstGameEl);

// do the same for the runner up item
const secondGameEl = document.createElement("p");
secondGameEl.innerText = second.name;
secondGameContainer.appendChild(secondGameEl);

// bonus: adding a search feature

const searchText = document.getElementById("search-input");

function searchGames() {
    const searchTerm = searchText.value;
    const filtered = GAMES_JSON.filter(game => game.name.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1);
    
    deleteChildElements(gamesContainer);
    addGamesToPage(filtered);
}

searchText.addEventListener("input", searchGames);
